...
export interface ISavingsGoalRepository {
  ...
  findActiveByUserAndType(userId: string, type: GoalType, year: number, month: number): Promise<SavingsGoal[]>;
  findById(id: string): Promise<SavingsGoal | null>;
  create(data: Pick<SavingsGoal, "userId" | "type" | "title" | "goalAmount" | "startYear" | "startMonth">): Promise<SavingsGoal>;
  update(id: string, data: Partial<Pick<SavingsGoal, "title" | "goalAmount">>): Promise<SavingsGoal>;
  archive(id: string, year: number, month: number): Promise<SavingsGoal>;
